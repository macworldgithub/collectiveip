      ...
      <div className="border-t border-gray-100 p-4">
        <div className="flex gap-2 mb-3 flex-wrap">
          {tool.sampleQuestions?.slice(0, 2).map((q) => (
            <button
              key={q}
              onClick={() => send(q)}
              className="text-xs px-3 py-1.5 bg-sky-50 text-sky-600 rounded-full hover:bg-sky-100 transition-colors border border-sky-200"
            >
        ...
        >
          <input
            ...
            className="flex-1 px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-sky-400 transition-colors"
          />
          <button
            type="submit"
            className="px-4 py-2.5 bg-sky-600 text-white rounded-lg hover:bg-sky-700 transition-colors"
          >
            <Send size={16} />
          </button>
        </form>
...
function AssessmentTool({ tool }: { tool: Tool }) {
  ...
  return submitted ? (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8 text-center">
      <div className="w-14 h-14 bg-sky-50 text-sky-600 rounded-full flex items-center justify-center mx-auto mb-4">
        {tool.icon}
      </div>
      <h3 className="text-xl font-bold text-gray-900 mb-2">Assessment Received</h3>
      ...
      <button
        ...
        className="px-5 py-2.5 bg-sky-600 text-white text-sm font-semibold rounded-md hover:bg-sky-700 transition-colors"
      >
        Start Again
      </button>
    </div>
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          ...
          <div key={f.label}>
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-700 focus:outline-none focus:border-sky-400 transition-colors bg-white"
              >
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-3 bg-sky-600 text-white font-semibold rounded-lg hover:bg-sky-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
      >
    ...
    <div className="font-sans text-gray-900 antialiased">
        ...
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-10">
            {tools.map((t) => (
              ...
              >
                <span
                  className={`flex-shrink-0 ${activeTool.id === t.id ? 'text-white' : 'text-sky-600'}`}
                >
                  {t.icon}
                </span>
                <div>
                  ...
                  <div
                    className={`text-xs ${activeTool.id === t.id ? 'text-sky-200' : 'text-gray-400'}`}
                  >
                    {t.subtitle}
                  </div>
                </div>
          ...
          {/* Tool Description */}
          <div className="mb-6">
            <p className="text-gray-500">{activeTool.desc}</p>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <a
            href="#"
            className="inline-flex items-center gap-2 px-7 py-3.5 bg-sky-600 text-white font-semibold rounded-md hover:bg-sky-700 transition-colors duration-200"
          >
